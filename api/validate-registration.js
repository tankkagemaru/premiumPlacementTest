...
//
// CHECK-ONLY endpoint. Confirms a registration code is valid (active, not
// expired, under max_uses) but does NOT increment its usage counter. Counter
// is incremented by /api/consume-registration-code AFTER Supabase Auth signup
// has actually created the user, so failed signups don't burn slots.
...
export default async function handler(req, res) {
  ...
  try {
    const { role, registrationCode } = req.body || {};
